...
func run() -> void:
	...
	var timer_line := {"mood":"happy", "text":"타이머 시작할게요~♪"}
	check(DialogueOutput.desktop_line(timer_line) == timer_line, "structured creator line decoded with mood")
	check(DialogueOutput.desktop_line(JSON.stringify(timer_line)) == timer_line, "serialized creator line decoded")
	check(DialogueOutput.desktop_line({"text":JSON.stringify(timer_line)}) == timer_line, "nested serialized timer line decoded")
	check(DialogueOutput.desktop_line('{"mood":"neutral","text":').is_empty(), "broken wrapper never shown")
	check(DialogueOutput.desktop_text(timer_line) == "(happy)타이머 시작할게요~♪", "creator mood remains an invisible expression control")
	check(DialogueOutput.desktop_line("[neutral] 안녕하세요.").get("text") == "[neutral] 안녕하세요.", "ordinary expression controls retained")
	var reactions := DesktopAIReactions.new()
	check(reactions._build_specific_fallback({"timer_start":[timer_line]}, "timer_start", "테스트", 25, "기본") == "(happy)타이머 시작할게요~♪", "actual timer fallback does not stringify dictionaries")
	var polite_profile := {"voice":{"casual":"차분한 존댓말을 사용한다."}}
	check(CharacterProfiles.uses_polite_speech(polite_profile), "legacy voice descriptions infer polite style")
	check(CharacterProfiles.default_ui_line_for_profile(polite_profile, "menu") == "필요하신 게 있나요?", "polite menu greeting")
	check(CharacterProfiles.default_ui_line_for_profile(polite_profile, "exit") == "다음에 또 뵐게요.", "polite farewell")
	polite_profile["default_lines"] = {"menu":"분부하실 일이 있으십니까?", "exit":"내일 다시 찾아뵙겠습니다."}
	check(CharacterProfiles.default_ui_line_for_profile(polite_profile, "exit") == "내일 다시 찾아뵙겠습니다.", "authored farewell takes priority")
	check(not CharacterProfiles.uses_polite_speech({"voice":{"register":"casual", "casual":"존댓말"}}), "explicit casual selection overrides inference")
	check(not CharacterProfiles.uses_polite_speech({"voice":{"casual":"존댓말을 쓰지 않는다."}}), "negated polite description not misclassified")
	check(reactions._build_specific_fallback({"_polite_defaults":true}, "timer_halfway", "테스트", 25, "절반 남았어.") == "절반 남았어요.", "polite timer default")
	reactions.free()
